#!/usr/bin/env python3
"""Run aggregate summaries for the exact p=h5phx URL marker."""
...
SQL_DIR = OUT_DIR / "sql"
RESULTS = OUT_DIR / "marker_summary_results.json"
RECEIPT = OUT_DIR / "marker_summary_receipt.json"
VALIDATOR = Path("/Users/robin/.codex/skills/waje-bigquery-readonly/scripts/validate_readonly_sql.py")
SQL_FILES = [
    SQL_DIR / "firebase_analytics_504208609_p_h5phx_summary.sql",
    SQL_DIR / "firebase_analytics_517134955_p_h5phx_summary.sql",
    SQL_DIR / "firebase_waje_ng_firebase_h5_p_h5phx_summary.sql",
]
...
def execute(client: bigquery.Client, path: Path) -> dict[str, Any]:
    ...
    item: dict[str, Any] = {"sql_file": str(path.relative_to(ROOT)), "query_location": "europe-west4", "sql_sha256": hashlib.sha256(sql.encode()).hexdigest(), "validation": validate(path)}
    if item["validation"]["status"] != "passed":
        item["status"] = "blocked_sql_validation"
        return item
    try:
        dry = client.query(sql, job_config=bigquery.QueryJobConfig(dry_run=True, use_query_cache=False, maximum_bytes_billed=5 * 1024**3), location="europe-west4")
        item["dry_run"] = {"status": "passed" if int(dry.total_bytes_processed or 0) <= 5 * 1024**3 else "blocked_cost", "total_bytes_processed": int(dry.total_bytes_processed or 0), "maximum_bytes_billed": 5 * 1024**3}
        if item["dry_run"]["status"] != "passed":
            item["status"] = "blocked_cost"
            return item
        job = client.query(sql, job_config=bigquery.QueryJobConfig(use_legacy_sql=False, use_query_cache=False, maximum_bytes_billed=5 * 1024**3), location="europe-west4")
...
def main() -> int:
    ...
    receipt: dict[str, Any] = {"run_id": "phenix_h5phx_marker_summary_2026_09_03", "project_id": "wajenigeria", "location": "europe-west4", "marker": "p=h5phx", "status": "not_started", "safety": {"aggregate_only": True, "raw_rows_saved": False, "raw_urls_saved": False, "parameter_values_saved": False, "user_identifiers_saved": False, "credentials_saved": False, "remote_systems_modified": False}}
    try:
        ...
        client = bigquery.Client(project="wajenigeria", credentials=creds, location="europe-west4")
    except Exception as exc:
    ...
    RECEIPT.write_text(json.dumps(receipt, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": receipt["status"], "query_count": receipt["query_count"], "failed_query_count": receipt["failed_query_count"], "dry_run_total_bytes": receipt["dry_run_total_bytes"], "results_file": receipt["results_file"]}, ensure_ascii=False, indent=2))
