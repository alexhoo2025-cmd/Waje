-- Data-platform scheduled refresh, not an Agent Runtime query.
-- Refreshes exact P95 metrics at two safe grains: overview and one diagnostic dimension.
...
CREATE TABLE IF NOT EXISTS `wajenigeria.waje_device_performance_mart.mart_native_performance_agent_overview_daily` (
  ...
  duration_trace_p50_ms FLOAT64,
  duration_trace_p95_ms FLOAT64,
  duration_trace_p99_ms FLOAT64,
  screen_trace_count INT64 NOT NULL,
  slow_frame_ratio FLOAT64,
  frozen_frame_ratio FLOAT64,
  ...
  network_p50_ms FLOAT64,
  network_p95_ms FLOAT64,
  network_p99_ms FLOAT64,
...
OPTIONS(description = 'Exact daily overview P50/P95/P99 aggregate facts for the Waje device-performance Agent. No trace names, URL or raw event fields.');

CREATE TABLE IF NOT EXISTS `wajenigeria.waje_device_performance_mart.mart_native_performance_agent_rank_daily` (
  metric_date_lagos DATE NOT NULL,
  endpoint STRING NOT NULL,
  platform STRING NOT NULL,
  app_package STRING NOT NULL,
  app_version STRING NOT NULL,
  analysis_dimension STRING NOT NULL,
  analysis_value STRING NOT NULL,
  performance_record_count INT64 NOT NULL,
  duration_trace_count INT64 NOT NULL,
  duration_trace_p95_ms FLOAT64,
  screen_trace_count INT64 NOT NULL,
...
PARTITION BY metric_date_lagos
CLUSTER BY endpoint, app_package, app_version, analysis_dimension, analysis_value
OPTIONS(description = 'Exact daily P95 and response-health aggregates for exactly one safe diagnostic dimension at a time.');

DELETE FROM `wajenigeria.waje_device_performance_mart.mart_native_performance_agent_overview_daily`
WHERE metric_date_lagos BETWEEN refresh_start_date AND refresh_end_date;

DELETE FROM `wajenigeria.waje_device_performance_mart.mart_native_performance_agent_rank_daily`
WHERE metric_date_lagos BETWEEN refresh_start_date AND refresh_end_date;

CREATE TEMP TABLE raw_performance AS
SELECT 'android_main' AS endpoint, 'Android' AS platform, 'com.hfhy.waje.special' AS app_package,
  event_timestamp, app_display_version, country, device_name, os_version, carrier, radio_type, event_type,
...
UNION ALL
SELECT 'android_transsion_old', 'Android', 'com.hfhy.wajecasino.palmgame',
  event_timestamp, app_display_version, country, device_name, os_version, carrier, radio_type, event_type,
...
WHERE DATE(event_timestamp, 'Africa/Lagos') BETWEEN refresh_start_date AND refresh_end_date
UNION ALL
SELECT 'android_transsion_new', 'Android', 'com.hfhy.wajecasino.game',
  event_timestamp, app_display_version, country, device_name, os_version, carrier, radio_type, event_type,
  network_info.response_code, SAFE_DIVIDE(network_info.response_completed_time_us, 1000.0),
  SAFE_DIVIDE(trace_info.duration_us, 1000.0), trace_info.screen_info.slow_frame_ratio,
  trace_info.screen_info.frozen_frame_ratio
...
WHERE DATE(event_timestamp, 'Africa/Lagos') BETWEEN refresh_start_date AND refresh_end_date
UNION ALL
SELECT 'ios_existing', 'iOS', 'com.wajegame.wajegame',
  event_timestamp, app_display_version, country, device_name, os_version, carrier, radio_type, event_type,
  network_info.response_code, SAFE_DIVIDE(network_info.response_completed_time_us, 1000.0),
  SAFE_DIVIDE(trace_info.duration_us, 1000.0), trace_info.screen_info.slow_frame_ratio,
  trace_info.screen_info.frozen_frame_ratio
...
INSERT INTO `wajenigeria.waje_device_performance_mart.mart_native_performance_agent_overview_daily`
SELECT
  ...
  AVG(IF(event_type = 'SCREEN_TRACE' AND slow_frame_ratio BETWEEN 0 AND 1, slow_frame_ratio, NULL)) AS slow_frame_ratio,
  AVG(IF(event_type = 'SCREEN_TRACE' AND frozen_frame_ratio BETWEEN 0 AND 1, frozen_frame_ratio, NULL)) AS frozen_frame_ratio,
  ...
  IF(COUNTIF(event_type = 'DURATION_TRACE' AND duration_ms >= 0) >= 500 AND COUNTIF(event_type = 'NETWORK_REQUEST' AND response_completed_ms >= 0) >= 500, 'provisional', 'immature') AS quality_status,
  'agent_v1' AS metric_definition_version,
  CURRENT_TIMESTAMP() AS refreshed_at
FROM raw_performance
WHERE event_type IN ('DURATION_TRACE', 'SCREEN_TRACE', 'NETWORK_REQUEST', 'TRACE_METRIC')
GROUP BY metric_date_lagos, endpoint, platform, app_package, app_version, complete_day;

INSERT INTO `wajenigeria.waje_device_performance_mart.mart_native_performance_agent_rank_daily`
WITH unpivoted AS (
  SELECT
    DATE(event_timestamp, 'Africa/Lagos') AS metric_date_lagos,
    endpoint,
  ...
  FROM raw_performance,
  UNNEST([
    STRUCT('device_name_bucket' AS analysis_dimension, COALESCE(NULLIF(device_name, ''), 'unknown') AS analysis_value),
...
SELECT
  ...
  IF(COUNT(*) >= 500, 'provisional', 'immature') AS quality_status,
  'agent_v1' AS metric_definition_version,
  CURRENT_TIMESTAMP() AS refreshed_at
FROM unpivoted
WHERE event_type IN ('DURATION_TRACE', 'SCREEN_TRACE', 'NETWORK_REQUEST', 'TRACE_METRIC')
GROUP BY metric_date_lagos, endpoint, platform, app_package, app_version, analysis_dimension, analysis_value, complete_day;
