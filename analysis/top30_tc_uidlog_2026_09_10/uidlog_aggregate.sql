...
events AS (
  SELECT
    ...
    COALESCE(NULLIF(e.log_event_type, ''), '(空)') AS extra_marker,
    e.change_count,
    e.change_balance,
    COALESCE(
...
enriched AS (
  SELECT
    ...
    event_time,
    CASE
      WHEN REGEXP_CONTAINS(LOWER(asset_method), r'充值|recharge|charge') THEN '充值'
      WHEN REGEXP_CONTAINS(LOWER(asset_method), r'提现|withdraw|audit') THEN '提现'
      WHEN REGEXP_CONTAINS(LOWER(asset_method), r'下注|\bbet\b') THEN '下注'
      WHEN REGEXP_CONTAINS(LOWER(asset_method), r'返还|退款|refund|return|结算|派奖|settle') THEN '返还/结算'
      WHEN REGEXP_CONTAINS(LOWER(asset_method), r'奖励|奖品|任务|签到|reward|prize|bonus') THEN '奖励'
      WHEN REGEXP_CONTAINS(LOWER(asset_method), r'转|兑换|exchange|transfer') THEN '转换'
      ELSE '其他'
    END AS reason_group,
    CASE
...
reason_rollup AS (
  SELECT
    ...
    MAX(event_time) AS last_event_time
  FROM enriched
  GROUP BY asset_id, reason_group
  HAVING COUNT(DISTINCT user_id) >= 10
),
time_rollup AS (
  SELECT
