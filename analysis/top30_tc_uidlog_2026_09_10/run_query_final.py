...
SQL=P/'uidlog_aggregate_final.sql'
...
(P/'query-receipt-final.json').write_text(json.dumps(receipt,ensure_ascii=False,indent=2,default=str))
...
if estimate>limit:
    receipt['status']='blocked_scan_limit';(P/'query-receipt-final.json').write_text(json.dumps(receipt,ensure_ascii=False,indent=2,default=str));raise SystemExit(2)
...
(P/'aggregate-result-final.json').write_text(json.dumps(result,ensure_ascii=False,indent=2,default=str))
(P/'query-receipt-final.json').write_text(json.dumps(receipt,ensure_ascii=False,indent=2,default=str))
