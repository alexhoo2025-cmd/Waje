...
events AS (
  SELECT
    ...
    e.asset_id,
    e.asset_source_id,
    COALESCE(NULLIF(e.asset_method, ''), '(空)') AS extra_marker,
    e.change_count,
    e.change_balance,
    COALESCE(
...
enriched AS (
  SELECT
    ...
    event_time,
    CASE
      WHEN REGEXP_CONTAINS(LOWER(extra_marker), r'充值|recharge|charge') THEN '充值'
      WHEN REGEXP_CONTAINS(LOWER(extra_marker), r'提现|withdraw|audit') THEN '提现'
      WHEN REGEXP_CONTAINS(LOWER(extra_marker), r'下注|\bbet\b') THEN '下注'
      WHEN REGEXP_CONTAINS(LOWER(extra_marker), r'返还|退款|refund|return|结算|派奖|settle|rchip|wcash') THEN '返还/结算'
      WHEN REGEXP_CONTAINS(LOWER(extra_marker), r'奖励|奖品|任务|签到|reward|prize|bonus') THEN '奖励'
      WHEN REGEXP_CONTAINS(LOWER(extra_marker), r'转|兑换|exchange|transfer') THEN '转换'
      ELSE '其他/待映射'
    END AS reason_group,
    CASE
...
reason_rollup AS (
  SELECT
    ...
    CONCAT(
      ...
      ' | marker=', SUBSTR(extra_marker, 1, 80)
    ) AS reason_key,
    '全天' AS time_key,
    COUNT(DISTINCT user_id) AS user_count,
    COUNT(*) AS event_count,
    ...
    MAX(event_time) AS last_event_time
  FROM enriched
  GROUP BY asset_id, reason_group, asset_source_id, extra_marker
  HAVING COUNT(DISTINCT user_id) >= 10
),
time_rollup AS (
  SELECT
