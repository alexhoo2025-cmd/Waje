...
(P/'query-receipt.json').write_text(json.dumps(receipt,ensure_ascii=False,indent=2,default=str))
...
if estimate>limit:
    receipt['status']='blocked_scan_limit';(P/'query-receipt.json').write_text(json.dumps(receipt,ensure_ascii=False,indent=2,default=str));raise SystemExit(2)
...
(P/'aggregate-result.json').write_text(json.dumps(result,ensure_ascii=False,indent=2,default=str))
(P/'query-receipt.json').write_text(json.dumps(receipt,ensure_ascii=False,indent=2,default=str))
