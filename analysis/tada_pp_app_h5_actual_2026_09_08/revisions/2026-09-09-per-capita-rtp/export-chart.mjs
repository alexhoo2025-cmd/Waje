...
const m=[...html.matchAll(re)].find(m=>m[1]==='per-bettor-chart-block');assert(m,'canonical SVG missing');
...
const legends=[...m[3].matchAll(/style="--portable-legend-color:([^"]+)"[^>]*><\/span><span>(.*?)<\/span>/g)];assert.equal(legends.length,2);
...
let x=width/2-90;for(const l of legends){svg+='<rect x="'+x+'" y="'+(h+35)+'" width="10" height="10" fill="'+l[1]+'"/><text x="'+(x+17)+'" y="'+(h+45)+'" font-family="Arial, PingFang SC, sans-serif" font-size="14" fill="#34465c">'+l[2]+'</text>';x+=100;}
svg+='</svg>';
const out='analysis/tada_pp_app_h5_actual_2026_09_08/lark-assets/per-bettor-chart-block';
