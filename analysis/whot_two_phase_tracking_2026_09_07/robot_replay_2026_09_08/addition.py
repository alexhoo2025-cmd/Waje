"""Requirement 16: observational bot outcome audit and full-round replay contract."""
def h(s): return ('h', 2, s)
def p(s): return ('p', s)
def t(headers, rows): return ('table', headers, rows)

OVERVIEW = [
 h('新增需求16｜机器人胜负偏离与整局回放'),
 p('目标：识别“必赢机器人最终输局”和“必输机器人最终赢局”，从开局发牌到最终裁决逐步回放真人与机器人的全部操作，定位规则执行、策略执行、状态同步或结果统计问题。作为二期新增第9项需求，原需求08—15继续保留。'),
 p('“必赢／必输”沿用策划的策略名称，代表配置的目标，不代表已经实现100%／0%胜率。按服务端实际下发并确认生效的目标判断；配置名、客户端表现、下注净输赢均不能单独决定目标或牌局胜负。本需求交付诊断数据与审计能力，策略变更须另行评审。'),
 t(['分析问题','可交付结果','证据要求'],[
 ...
 p('采集路径：开局固化席位与策略 → 全程记录状态变化及动作 → 服务端裁决识别偏离 → 固化整局轨迹 → 回放校验 → 按版本和结束原因复盘。必须从开局采集；结束时才开始记录无法补回先前的摸牌和出牌。'),
 p('范围：所有经服务端确认含机器人的新版Whot牌局。机器人席位是本模块统计主体；真人手动、真人托管、机器人控制分别记录。请求人数2/4与实际人数2/3/4独立保存；多人局或多机器人组合是否允许该策略，由版本化适用范围确认。超出适用范围仍保留记录，并标记scope_conflict。'),
 p('依赖复用：需求08托管/退出、09 Last Card、11同分抽牌、12网络恢复、13结束与结算，通过game_round_id、seat_id、cause_event_uid关联到同一轨迹；沿用一期端、包、模式、房间、付费池、规则版本。普通运营报表继续真人为主，本模块的机器人指标另列，避免混用分母。'),
]

METRICS = [
 h('需求16｜统计对象、核心指标及算法'),
 p('统计对象分为整桌牌局与机器人席位局。整桌键为game_round_id；机器人席位局键为game_round_id＋seat_id，同一机器人账号跨局不合并。按开局时间归属Lagos业务日／小时，保存截至时间；牌局终结与日志入库缓冲结束后计算。未终结、作废、结果未知单列。'),
 p('可判定集合E：已终结有效牌局，机器人身份明确，服务端策略在开局前或首个策略动作前生效，目标为win/lose且全局未切换，规则及策略适用范围明确，存在唯一最终胜者。回放缺失不从E中删除；它影响诊断能力，不应使发生率变低。'),
 t(['核心指标','算法与分母','解释与来源'],[
 ...
 p('分类规则：actual_result=win当且仅当该席位为服务端唯一最终胜者；有另一位唯一胜者时为lose。同分决胜尚未完成为pending，取消退款为void，多个胜者或裁决缺失为unknown。中途换策略保留segment轨迹，但整局单列policy_changed，不用最后一次目标回写开局目标。若策略目标实际指“指定真人输赢”而非机器人自己，需记录target_subject与目标席位并另算，不能套用本表。'),
 p('必选维度：robot_policy_version、config_version、ruleset_version、服务端版本、目标、实际人数、人机组成、房间、付费池、结束原因、Lagos小时。APP/H5按同桌真人端组合展示；按参与端展开会重复桌数，汇总重新去重。真人新老／付费分组沿用一期。概率配置按该局生效快照分析，不把策略文档默认概率硬编码为生产事实。'),
 p('三条终局路径分别分析：手牌出完；牌堆耗尽且最低积分唯一；最低同分后抽牌决胜。没有唯一结果、规则映射不明或分母为0时显示N/A。需要展示真人群体的聚合结果时，少于10人的分组隐藏或合并；受控回放用席位代号，不公开账户或设备标识。'),
]
...
IMPLEMENTATION = [
 h('需求16｜机器人胜负偏离与整局回放实现'),
 p('以下为新增逻辑事件合同，最终event_id由埋点管理员映射。完整手牌、牌堆顺序与决策轨迹进入服务端受控回放存储；分析层仅接收牌局摘要、结果分类与replay_id。客户端不承载完整对手手牌。已存在的二期事件关联复用，不重复上报同一状态事实。'),
 t(['事件','责任与触发','专属字段','幂等与校验'], EVENTS),
 t(['公共字段组','类型与必填条件','用途'],[
 ...
 h('需求16｜采集、存储与回放流程'),
 p('1. 开局为全部含机器人牌局分配replay_id并开启完整服务端轨迹，异步写入可靠队列或事务outbox；每条状态变化带同一牌局序号和前后hash。需要提前固定顺序的洗牌/RNG结果、外部响应及实际状态变动一并保存。日志失败记录并告警，不要求客户端等待回放归档。'),
 p('2. 持久化层将整局日志按序分块，生成包含起止序号、总事件数、分块校验和、规则与引擎版本的manifest。初始状态＋所有变化量足以重建，每个关键阶段加检查点。断线恢复只引用检查点；时间戳、手牌数量或最终截图均不能替代状态日志。'),
 p('3. 终局生成所有机器人席位局的全量摘要，先分类偏离再决定长期留存。两类偏离及目标未知／策略冲突／执行失败的局建议100%保留；正常对照按策略版本×实际人数×结束原因固定分层抽样，记录sample_rate及seed。所有比率用全量摘要，不能用异常回放库算发生率。'),
 p('4. 短期轨迹从开局持续保留至终局判定与归档确认完成，异常长局不得被TTL提前删除；未收到归档成功回执时重试并标记缺口。建议待确认的留存配置：普通完整轨迹7天、偏离与对照回放30天；结果更正若已过保留期，更新分类并标记replay_expired，不能补造轨迹。'),
 p('5. 回放校验分两层：结构完整性（无断序、重复计数、缺快照、校验和错误）与状态重放（按该局引擎/规则重放后每步hash及最终胜者一致）。策略决策复验为独立状态，依赖策略版本、决策输入与随机输出；不能仅凭动作重放成功就认定策略正确。'),
 p('6. 研发回放页建议：顶部显示偏离类型、目标与实际结果、版本、结束原因和完整性；中部按席位显示手牌/牌堆/弃牌及逐步时间线；侧栏显示决策、实际执行、状态变化和首个差异。支持播放、暂停、步进、跳转Last Card/决胜/首差异；数据不完整时显示缺口位置。只读复盘不调用生产动作或资金接口。'),
 h('需求16｜原因分类与评审验收'),
 t(['原因类别','可以确认的证据','处理建议'],[
 ...
 p('必测：必赢输/赢、必输赢/输、目标未知、下发未生效、中途切换、同桌多机器人两类同时命中；2/3/4人合法与不适用组合；三种终局路径及多轮同分；20图案与宣告时序；2反弹链、14、1/8连出；多对手并发抓罚只罚一次；真人托管恢复、模拟离线与真实断线；牌库变化及展示前手牌变化；传输重复/乱序/缺片、服务重启、归档失败重试、过期更正与结果版本升级。'),
 p('验收例：一桌A为win目标机器人但输、B为lose目标机器人却赢，统计为2个偏离席位局、1个偏离牌局；3局偏离中2局完整重放通过、1局缺片，核心回放通过率为2/3。若win目标有效席位局10个、输2个，其中1个回放缺失，输局率仍为2/10。以上均为合成验收例，不是线上数据。'),
 p('上线验收建议：固定测试牌局100%可重放且最终结果一致，偏离分类与人工裁决一致，同一业务事实重复计数为0；生产缺帧、未归档和未知原因持续单列。策划确认目标主体、多人适用范围和策略冲突优先级，服务端确认权威状态与版本快照，数据团队验收分母及更正逻辑，测试验收双端表现与单一服务端事实。'),
 p('来源与待确认：2026年9月8日用户新增需求；《Whot 6001 真金博彩机器人策略》revision 69与新版玩法需求revision 5738已重新读取。策略文档参数表未作为线上生效证据；新版本适用规则、目标主体、运行配置、日志现有覆盖和留存期限需研发／策划确认。本文是埋点及回放设计，尚未采集任何真实牌局或部署服务。'),
]

GROUP = {'requirement_id':16,'name':'机器人胜负偏离与整局回放','takeaway':'识别两类胜负偏离，完整记录真人与机器人轨迹并校验回放。','events':[
 {'logical_event':r[0],'source':r[1],'fields':r[2],'trigger':r[1],'metric_definition':r[3]} for r in EVENTS
]}
