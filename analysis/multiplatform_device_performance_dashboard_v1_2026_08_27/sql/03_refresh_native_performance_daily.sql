-- D+1 rolling refresh: 30 Lagos business days. Raw performance event_name is intentionally not selected.
...
CREATE TABLE IF NOT EXISTS `wajenigeria.waje_device_performance_mart.mart_native_performance_daily` (
  ...
  carrier_bucket STRING NOT NULL,
...
CLUSTER BY endpoint, app_package, app_version, country, device_name
OPTIONS(description = 'Aggregate-only Android/iOS Firebase Performance daily facts. Trace names and network URL patterns are intentionally excluded.');

DELETE FROM `wajenigeria.waje_device_performance_mart.mart_native_performance_daily`
WHERE metric_date_lagos BETWEEN refresh_start_date AND refresh_end_date;

INSERT INTO `wajenigeria.waje_device_performance_mart.mart_native_performance_daily`
...
SELECT
  ...
  IF(
    ...
    APPROX_QUANTILES(IF(event_type = 'DURATION_TRACE' AND duration_ms >= 0, duration_ms, NULL), 100 IGNORE NULLS)[SAFE_OFFSET(95)],
    NULL
  ) AS duration_trace_p95_ms,
  ...
  IF(
    ...
    APPROX_QUANTILES(IF(event_type = 'NETWORK_REQUEST' AND response_completed_ms >= 0, response_completed_ms, NULL), 100 IGNORE NULLS)[SAFE_OFFSET(95)],
    NULL
  ) AS network_p95_ms,
  MAX(event_timestamp) AS data_cutoff_at,
  metric_date_lagos < CURRENT_DATE('Africa/Lagos') AS complete_day,
  CASE
    WHEN metric_date_lagos >= CURRENT_DATE('Africa/Lagos') - 6 THEN 'immature'
    ELSE 'provisional'
...
FROM eligible
GROUP BY metric_date_lagos, endpoint, platform, app_package, app_version, country, device_name, os_version, carrier_bucket, network_type, complete_day, quality_status;
