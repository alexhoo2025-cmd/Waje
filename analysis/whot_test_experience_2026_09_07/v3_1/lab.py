...
GAMES = ('6001', '9006')
VERSION = 'visible-controller-v3.1'
...
def gate(context, profile):
    ...
    reasons = []
    expected_origin = urlparse(profile.get('origin', 'https://test-h5.wajew.com'))
    if (u.scheme != expected_origin.scheme or u.hostname != expected_origin.hostname
            or u.port not in (None, 443) or expected_origin.port not in (None, 443)):
...
def decide(state, rules, arm, at_ms=None):
    ...
    if state.get('autoplay') is not False:
        return end('rescan', 'control_mode_unknown')
    ...
    if pending != 'none':
        ...
        return end(effect['action'], 'verified_special_window', expected_draw_count=effect.get('draw_count',1))
...
def eligibility(meta, settlement, events):
    ...
    if settlement.get('end_reason') not in ('hand_empty', 'opponent_hand_empty', 'deck_exhausted', 'tie_break'):
    ...
    bad_quality = {'mixed_auto_play','controller_stopped_unqualified','uncontrolled_after_local_loop_stop',
                   'uncertified_manual_intervention','mixed_or_unverified_control'}
    failures = {'autoplay','capture_gap','invalid_scope','unresolved_state','manual_intervention','action_unknown'}
    if settlement.get('quality') in bad_quality or any(
        e.get('type') in failures or e.get('quality') in bad_quality or
        (e.get('type') == 'stop' and e.get('reason') not in ('settlement_complete',)) for e in events):
    ...
    submissions = {e.get('decision_id') for e in events if e.get('type') == 'action_submitted'}
    ...
    good = [e for e in actions if e.get('accepted') is True and e.get('legal') is True
            ...
            and e.get('decision_id') in submissions and e.get('turn_id') in opportunities]
    if submissions != {e.get('decision_id') for e in good} or len(actions) != len({e.get('decision_id') for e in actions}):
        reasons.append('submission_ack_mismatch')
    denominator = settlement.get('observed_turn_opportunities')
    review = settlement.get('coverage_review') or {}
    continuous = (settlement.get('continuous_capture_verified') is True
                  and review.get('method') == 'independent_continuous_replay'
                  and bool(review.get('evidence_sha256'))
                  and review.get('opportunity_ids') == sorted(opportunities))
