...
class LabTests(unittest.TestCase):
    ...
    def state(self):
        return dict(client_game_id='6001',state_seq=4,foreground=True,window_verified=True,
                    ...
                    action_owner='self',autoplay=False,pending_effect='none',table_card={'rank':3,'shape':'circle'},
    ...
    def valid_events(self):
        return [{'type':'match_start'}, {'type':'turn_open','turn_id':'t1'},
                ...
                {'type':'action_submitted','decision_id':'d1'},
    ...
    def settlement(self):
        return dict(visible=True,result='win',end_reason='hand_empty',observed_turn_opportunities=1,
                    continuous_capture_verified=True,coverage_review={'method':'independent_continuous_replay',
                    'evidence_sha256':'a'*64,'opportunity_ids':['t1']})
    ...
    def test_schedule_recovery_and_atomic_settlement(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            store.append(id_,5,self.valid_events()[-1])
            self.assertEqual(store.c.execute('select count(*) from events').fetchone()[0],5)
