    ...
    func frame(windowID id:UInt32)throws -> (CGImage,Double,Int) {
        ...
        while true {
            ...
            if let buffer=buffer {
                ...
                guard age<=500 else {throw Fault.invalid("capture_stream_stale")}
...
func templateNumbers(_ image:CGImage,_ roi:[Double])throws->[[String:Any]] {
    ...
    for piece in pieces {
        ...
        group.append((scores[0].0,b,1-scores[0].1))
    }
    flush();return out
}
...
func handle(_ q:[String:Any]) throws -> [String:Any] {
    ...
    if op == "status" {
        var result:[String:Any] = ["input_enabled":inputEnabled,"windows":windows(),
            "foreground_chrome":NSWorkspace.shared.frontmostApplication?.bundleIdentifier == "com.google.Chrome",
    ...
    if op == "inspect" {
        ...
        let wins=focused.map { [$0 as! AXUIElement] } ?? []
    ...
    if op == "click" {
        ...
        guard let pid=w["pid"] as? Int,NSWorkspace.shared.frontmostApplication?.processIdentifier==pid_t(pid),
    ...
    return ["regions":try analyze(i,q["regions"] as? [String:[Double]] ?? [:]),"width":i.width,"height":i.height,"frame_age_ms":sampled.1,"capture_seq":sampled.2]
}
