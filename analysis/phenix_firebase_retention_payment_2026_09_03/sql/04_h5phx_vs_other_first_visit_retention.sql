-- Aggregate-only diagnostic: Phoenix versus other first-visit cohorts in the same Firebase H5 stream.
-- It returns segment/date counts only; no identifiers, URLs, or parameter values.
WITH raw_events AS (
  SELECT
...
first_visit AS (
  SELECT
    ...
    CASE
      WHEN h5phx_marker THEN 'Phoenix p=h5phx'
      WHEN LOWER(COALESCE(first_source, '')) = '(direct)'
       AND LOWER(COALESCE(first_medium, '')) = '(none)' THEN '其他直接访问'
      ELSE '其他有归因访问'
