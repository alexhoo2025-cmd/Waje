-- Aggregate-only strict H5 natural short-horizon payment lifecycle template.
-- Replace cohort and payment date literals per disjoint cohort window.
WITH cohort_base AS (
  SELECT DISTINCT
    target_day AS cohort_date,
    user_id
  FROM `wajenigeria.origin_hfyl.user_xlid`
  WHERE target_day BETWEEN DATE '2026-07-16' AND DATE '2026-07-31'
  ...
  FROM raw_orders
  WHERE payment_date BETWEEN DATE '2026-07-16' AND DATE '2026-08-13'
