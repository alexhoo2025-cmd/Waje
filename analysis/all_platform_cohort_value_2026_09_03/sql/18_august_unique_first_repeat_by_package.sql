-- Aggregate-only August unique first/repeat payer counts by first platform and package.
-- Deliberately excludes registration and channel fields to keep the query within the 5 GiB guardrail.
WITH raw_orders AS (
  SELECT
  ...
  GROUP BY successful_orders.user_id, first_pay_date, first_package_name, first_client_type
)
SELECT
...
FROM payer_month
GROUP BY platform, first_package_name
HAVING COUNT(*) >= 10
