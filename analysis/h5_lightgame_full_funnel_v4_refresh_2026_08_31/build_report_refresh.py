...
for row in return_rows:
    ...
    if entry_date <= "2026-08-25":
        item["d1_den"] += users
        item["d1_num"] += int(row["return_d1_users"])
    if entry_date <= "2026-08-23":
        item["d3_den"] += users
...
for row in new_summary:
    ...
    if cohort <= "2026-08-25":
        item["d1_den"] += users
        item["d1_num"] += int(row["return_d1_users"])
    if cohort <= "2026-08-23":
        item["d3_den"] += users
        item["d3_num"] += int(row["return_d3_users"])
source_summary = []
...
quality_matrix = [
    {"item": "GA4日表连续性", "status": "provisional", "evidence": "2026-08-20至08-26连续7日；每日24类事件；全部WEB"},
...
(ROOT / "source_register.json").write_text(json.dumps({
    "ga4_bigquery": {"project": "waje-analytics-readonly", "dataset": "analytics_504208609", "cutoff": "2026-08-26", "status": "provisional"},
...
draw.text((125, 735), "共同完整日：2026-08-26｜Hilo/Plinko仅具备D1和部分D3；D7显示N/A", fill=INK, font=font(25, True))
save_chart(image, "01_轻量化游戏发布与分析节点.png")


# Chart 2: game page reach and intensity
image, draw = canvas("GA4首次访问用户的轻量化游戏页面触达", "2026-08-20至08-26；页面进入不是游戏开始，用户可能进入多款游戏")
...
image, draw = canvas("进入轻量化游戏页的新访客：D1/D3应用回访", "同一GA4首次访问批次；D1截至8/25、D3截至8/23；D7尚未成熟")
