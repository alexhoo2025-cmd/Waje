...
function EvidenceTable({ id, title, queryId, rows, displayRows, columns, caption, searchable = false }) {
  const amountFields = new Set(['相对基线多派奖','当日下注','当日平台净盈利','下注额','派奖额','平台净盈利','玩家净赢','Currency下注','Lifecycle下注','Currency盈利','Lifecycle盈利']);
  const formattedColumns = columns.map((column) => {
    const values = displayRows.map(row => row[column.field]).filter(v => typeof v === 'number' && Number.isFinite(v));
    const eligible = values.length > 1 && column.field !== '排名' && Math.max(...values) !== Math.min(...values);
    const max = eligible ? Math.max(...values) : null, min = eligible ? Math.min(...values) : null;
    return {...column, renderCell: (value, row) => {
      const label = column.renderCell ? column.renderCell(value) : amountFields.has(column.field) ? amount(value) : value;
      const extreme = eligible && value === max ? '最高' : eligible && value === min && min !== 0 ? '最低' : null;
      const focus = row.排名 === 1 || row.对比 === '9月10日';
      return <span className={`evidence-value ${extreme ? 'evidence-extreme' : focus ? 'evidence-focus' : ''}`}>
        {label ?? '—'}{extreme && <small className="extreme-label">{extreme}</small>}
      </span>;
    }};
  });
  return <DataComponent id={id} title={title} queryId={queryId} kind="table"
    ...
    <DataTable rows={displayRows} columns={formattedColumns} caption={caption} searchable={searchable} />
    <p className="extreme-key">最高／最低按本表全部行逐列比较，包含分页外数据；标记表示数值极值。</p>
  </DataComponent>;
}
...
export function ReportContent() {
  ...
  const largestOffset = [...drivers].sort((a,b) => a.excess_payout-b.excess_payout)[0];
  ...
  const heatPeak = [...heatmap].filter(r => Number.isFinite(r.rtp)).sort((a,b) => b.rtp-a.rtp)[0];
  const heatLow = [...heatmap].filter(r => Number.isFinite(r.rtp)).sort((a,b) => a.rtp-b.rtp)[0];
  const driverTable = drivers.slice(0, 12).map((row, index) => ({
    排名: index + 1, 游戏: row.game, "9月10日RTP": row.sep10_rtp, "前5日RTP": row.baseline_rtp,
    RTP变化: row.rtp_change_pp, 相对基线多派奖: row.excess_payout,
    当日下注: row.sep10_bet, 当日平台净盈利: row.sep10_net_win,
  }));
  const moneyTable = [
    ...
    { 对比: "变化", 成功充值: change(move.recharge_change), 成功提现: change(move.withdraw_change), TC: pp(move.tc_change_pp) },
  ];
  const reconTable = reconciliation.map((row) => ({
    日期: row.date, Currency下注: row.vendor_bet, Lifecycle下注: row.lifecycle_bet,
    下注差异: row.bet_difference_pct, Currency盈利: row.vendor_profit,
    Lifecycle盈利: row.lifecycle_profit, 盈利差异: row.profit_difference_pct,
  }));
  const gameTable = [...games].sort((a, b) => b.bet - a.bet).map((row) => ({
    排名: row.bet_rank, 游戏: row.game, 类型: row.game_type, 下注额: row.bet,
    下注份额: row.bet_share, 派奖额: row.win, 平台净盈利: row.net_win,
    玩家净赢: row.player_net_gain, 加权RTP: row.rtp, 局数: row.total_count,
  }));
...
**提现增幅超过充值，推动TC达到观察期高点。** 9月10日成功提现由前5日日均${amount(avg("withdraw"))}增至 **${amount(sep10?.withdraw)}**，增长 **${change(move.withdraw_change)}**；成功充值由${amount(avg("recharge"))}增至 **${amount(sep10?.recharge)}**，增长 **${change(move.recharge_change)}**。提现多增 **${amount(move.withdraw_increase - move.recharge_increase)}**，TC由${pct(priorTc)}升至 **${pct(sep10?.tc_rate)}**。`} />
      </ReportSection>
        ...
        <EvidenceChart id="tc-rtp-link" queryId="tc_daily" title="每日Tada参与用户TC与游戏RTP" rows={rateRows} sourceRows={daily}
          height={300} spec={{ type: "line", x: "date", y: "游戏加权RTP（%）", fields: ["游戏加权RTP（%）", "Tada参与用户TC（%）"],
            valueDecimals: 2, colors: { "游戏加权RTP（%）": "var(--chart-1)", "Tada参与用户TC（%）": "var(--chart-3)" }, annotations: [
              ...
              { id: "tc-low", kind: "point", at: "2026-09-06", field: "Tada参与用户TC（%）", label: "TC低点 77.60%" },
              { id: "rtp-high", kind: "point", at: "2026-09-10", field: "游戏加权RTP（%）", label: "RTP峰值 97.26%" },
      ...
      <EvidenceTable id="money-table" title="TC核心数据" queryId="tc_daily" rows={daily} displayRows={moneyTable}
        caption="TC按充值与提现金额加权；9月11日部分日不参与对比。" columns={[
          ...
          { field: "TC", label: "TC", renderCell: (v) => typeof v === "number" ? pct(v) : <strong className="evidence-extreme">{v}</strong> },
        ]} />
    </section>
...
**同款游戏回报变化贡献了大部分RTP抬升。** 9月10日Tada加权RTP较前5日上升 **${pp(decomp.total_change_pp)}**。其中游戏结构变化贡献${pp(decomp.mix_effect_pp)}，同款游戏RTP变化贡献 **${pp(decomp.within_game_effect_pp)}**，占总抬升的 **${pct(decomp.within_game_effect_share, 1)}**。
...
各游戏正向多派奖合计 **${amount(decomp.positive_excess_payout)}**，被其他游戏少派奖 **${amount(Math.abs(decomp.negative_excess_payout))}** 抵消后，净多派奖 **${amount(decomp.net_excess_payout)}**。`} />
      ...
      <EvidenceChart id="rtp-driver" queryId="rtp_drivers" title="9月10日各游戏相对自身RTP基线的派奖偏差" rows={driverRows} sourceRows={drivers}
        ...
        <RichNarrative id="driver:extrema" className="report-analysis peak-callout" label="编辑贡献极值说明"
          value={`**最大多派奖：${topDriver.game}，${amount(topDriver.excess_payout)}。** 最大抵消项：**${largestOffset.game}，少派奖${amount(Math.abs(largestOffset.excess_payout))}**。均相对同款游戏前5日RTP基线。`} />
      </EvidenceChart>
...
**3 Lucky Chong Tian Pao的多派奖贡献最大。** 9月10日该游戏下注 **${amount(topDriver?.sep10_bet)}**，RTP由前5日${pct(topDriver?.baseline_rtp)}升至 **${pct(topDriver?.sep10_rtp)}**。相对自身基线多派奖 **${amount(topDriver?.excess_payout)}**，当日玩家净赢 **${amount(Math.max(-(topDriver?.sep10_net_win ?? 0), 0))}**。

第二至第四位为 **${drivers[1]?.game}**、**${drivers[2]?.game}** 和 **${drivers[3]?.game}**。作为反例，下注规模最大的 **X7 HOT** 当日相对自身基线少派奖 **${amount(Math.abs(x7?.excess_payout))}**，不是本次RTP升高来源。`} />
      </ReportSection>
      <EvidenceChart id="rtp-heatmap" queryId="heatmap" title="重点游戏逐日RTP" rows={heatRows} sourceRows={heatmap}
        ...
        <RichNarrative id="heatmap:extrema" className="report-analysis peak-callout" label="编辑峰值说明"
          value={`**本图最高RTP：${heatPeak.game} · ${heatPeak.date} · ${pct(heatPeak.rtp)}。** 最低为 **${heatLow.game} · ${heatLow.date} · ${pct(heatLow.rtp)}**。极值仅限本图所列游戏与日期，9月11日为部分日。`} />
      </EvidenceChart>
    </section>
    ...
    <ReportSection id="conclusion" title="结论与行动" queryId="tc_daily"
      ...
      <RichNarrative id="conclusion:body" className="report-caveat" label="编辑结论与行动" value={`## 04｜结论：游戏RTP异常是核心原因，但不是全部原因

**分析判断（待验证）：将游戏RTP异常作为TC异常的核心原因假设，仍需核验派奖到提现的资金关联。**

**现有证据支持优先复核该游戏，尚不能确认提现归因。** 完整日RTP与TC的Pearson相关为 **${corr.pearson_rtp_tc?.toFixed(2)}**、Spearman为 **${corr.spearman_rtp_tc?.toFixed(2)}**，但只有 **6天** 且共同受9月10日高点影响。游戏侧净多派奖仅相当于提现增量的 **${pct(decomp.net_excess_vs_withdraw_increase, 1)}**，不能提现直接归因给3 Lucky Chong Tian Pao。

1. **先复核3 Lucky Chong Tian Pao。** 检查最终派奖、取消退款、Bonus、版本和高倍结果。
2. **再追踪完整日TC。** 若后续仍高于9月5—9日水平，再拆解提现人数、金额分布和资金节奏。
3. **统一两套Tada金额口径。** Currency Summary与GM Lifecycle的每日下注差异为 **1.78%—5.41%**，未确认时区、结算状态和Bonus范围前不合并。`} />
    </ReportSection>
