WITH user_day AS (
  ...
  WHERE target_day = DATE '2026-09-11'
    AND SAFE_CAST(server_time AS INT64) <= 1789126793000
    AND app_id = 90006
...
SELECT
  ...
  recharge,
  withdraw,
  SAFE_DIVIDE(withdraw, recharge) AS tc_rate,
  duplicate_recharge_rows,
  duplicate_withdraw_rows
FROM daily
ORDER BY biz_date
LIMIT 100;
