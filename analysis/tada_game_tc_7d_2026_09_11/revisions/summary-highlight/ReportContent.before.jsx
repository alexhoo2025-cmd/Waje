...
- **TC异常的直接原因是提现增长快于充值。** 9月10日Tada参与用户提现较前5日日均增加${change(move.withdraw_change)}，充值增加${change(move.recharge_change)}，TC升至${pct(sep10?.tc_rate)}，提高${pp(move.tc_change_pp)}。
- **游戏侧RTP异常集中于3 Lucky Chong Tian Pao。** Tada加权RTP提高${pp(decomp.total_change_pp)}；其中${pct(decomp.within_game_effect_share, 1)}来自同款游戏RTP变化。3 Lucky Chong Tian Pao相对自身基线多派奖${amount(topDriver?.excess_payout)}，是最大贡献项。
