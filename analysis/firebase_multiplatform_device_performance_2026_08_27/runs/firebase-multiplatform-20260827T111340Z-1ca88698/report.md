...
**当前状态：`quality_warning`。** 至少一个 Firebase 数据源或查询缺失、延迟或受阻
...
| iOS | 0 Analytics 事件；2.45M Performance 记录 | 会话开始事件、轨迹/网络 P95、屏幕流畅度 | `immature` / `provisional` |
...
| query:android_analytics_daily | `failed` | Error in query string: SELECT list expression references column app_version
which is neither grouped nor aggregated at [17:3]

 |
| query:ios_analytics_daily | `failed` | Error in query string: SELECT list expression references column app_version
which is neither grouped nor aggregated at [17:3]
