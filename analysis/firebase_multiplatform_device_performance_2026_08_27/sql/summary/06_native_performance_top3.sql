-- Compact native Performance dimension ranking: top 3 aggregate values per
-- endpoint/package/dimension across the window. No date/version detail.
WITH raw_performance AS (
  SELECT 'android_main' AS endpoint, 'Android' AS platform, 'com.hfhy.waje.special' AS app_package, event_timestamp, country, device_name, os_version, carrier, radio_type, event_type, network_info.response_code AS response_code, SAFE_DIVIDE(network_info.response_completed_time_us, 1000.0) AS response_completed_ms, SAFE_DIVIDE(trace_info.duration_us, 1000.0) AS duration_ms
  ...
  UNION ALL
  SELECT 'android_transsion_old', 'Android', 'com.hfhy.wajecasino.palmgame', event_timestamp, country, device_name, os_version, carrier, radio_type, event_type, network_info.response_code, SAFE_DIVIDE(network_info.response_completed_time_us, 1000.0), SAFE_DIVIDE(trace_info.duration_us, 1000.0)
  FROM `wajenigeria.waje_ng_firebase_android_performance.com_hfhy_wajecasino_palmgame_ANDROID`
  WHERE DATE(event_timestamp, 'Africa/Lagos') BETWEEN DATE '__PERF_START__' AND DATE '__PERF_END__'
  UNION ALL
  SELECT 'android_transsion_new', 'Android', 'com.hfhy.wajecasino.game', event_timestamp, country, device_name, os_version, carrier, radio_type, event_type, network_info.response_code, SAFE_DIVIDE(network_info.response_completed_time_us, 1000.0), SAFE_DIVIDE(trace_info.duration_us, 1000.0)
  FROM `wajenigeria.waje_ng_firebase_android_performance.com_hfhy_wajecasino_game_ANDROID`
  WHERE DATE(event_timestamp, 'Africa/Lagos') BETWEEN DATE '__PERF_START__' AND DATE '__PERF_END__'
  UNION ALL
  SELECT 'ios_existing', 'iOS', 'com.wajegame.wajegame', event_timestamp, country, device_name, os_version, carrier, radio_type, event_type, network_info.response_code, SAFE_DIVIDE(network_info.response_completed_time_us, 1000.0), SAFE_DIVIDE(trace_info.duration_us, 1000.0)
  FROM `wajenigeria.waje_ng_firebase_ios_performance.com_wajegame_wajegame_IOS`
  WHERE DATE(event_timestamp, 'Africa/Lagos') BETWEEN DATE '__PERF_START__' AND DATE '__PERF_END__'
), expanded AS (
  SELECT endpoint, platform, app_package, dimension.rank_dimension, dimension.rank_value, event_type, response_code, response_completed_ms, duration_ms
  ...
  SELECT
    endpoint, platform, app_package, 'all_versions' AS app_version,
    rank_dimension, rank_value,
...
SELECT endpoint, platform, app_package, app_version, rank_dimension, rank_value,
