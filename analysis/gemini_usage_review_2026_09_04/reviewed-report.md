...
- Gemini 网页 Agent 保留为可用专项通道；遇认证问题恢复同一授权会话后继续。仅本地 CLI 桥接保持 recovery-probe 恢复门禁，不停用网页通道。
- 本地 CLI 恢复顺序：企业身份 → Vertex推理 → 允许视图/只读查询（BQ任务）→ 真实结构化结果 → 下游消费回读。网页偶发登录验证不要求重走整套 CLI 恢复流程。
...
已由用户补充确认：网页Agent执行效果较好，偶尔需要账户和凭证验证，并非重大问题。以上Claude分析保留为此前基于本地CLI记录的历史判断；不能据此停用网页通道。仍待定位的仅是本地CLI超时与IAM问题。
