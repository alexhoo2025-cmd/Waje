...
SELECT target_day AS activity_date,xl_id FROM `wajenigeria.origin_hfyl.view_event_server` WHERE target_day BETWEEN DATE '2026-08-16' AND DATE '2026-08-29' AND app_id=90006 AND event_type IN ('LOGIN','LOGOUT','REGISTER')
UNION ALL
SELECT target_day AS activity_date,xl_id FROM `wajenigeria.origin_hfyl.view_event_client` WHERE target_day BETWEEN DATE '2026-08-16' AND DATE '2026-08-29' AND app_id=90006 AND event_type IN ('LOGIN','LOGOUT','REGISTER','AL','AQ')
UNION ALL
SELECT target_day AS activity_date,xl_id FROM `wajenigeria.origin_hfyl.view_event_web` WHERE target_day BETWEEN DATE '2026-08-16' AND DATE '2026-08-29' AND app_id=90006 AND event_type IN ('AL','AQ','PD')
...
WHERE p.observation_date BETWEEN DATE '2026-08-16' AND DATE '2026-08-29'
