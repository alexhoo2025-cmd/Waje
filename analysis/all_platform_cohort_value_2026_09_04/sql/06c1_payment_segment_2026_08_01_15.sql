-- Aggregate-only August 1-15 payer lifecycle segmentation by first package/channel.
WITH raw_orders AS (
  ...
  WHERE target_day BETWEEN DATE '2026-08-01' AND DATE '2026-08-15' AND event_type = 'order_success'
), successful_orders AS (
  SELECT payment_date, user_id, order_no, MAX(COALESCE(pay_amount, 0)) AS order_amount
  FROM raw_orders WHERE cohort_date BETWEEN DATE '2026-08-01' AND DATE '2026-08-15'
...
SELECT '2026-08-01/15' AS payment_window,
  ...
  COUNTIF(register_date BETWEEN DATE '2026-08-01' AND DATE '2026-08-15') AS unique_new_registered_payers,
