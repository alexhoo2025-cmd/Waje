-- Aggregate-only June payer lifecycle segmentation by first package and channel.
-- Success orders are deduplicated by day, user, and order number.
WITH raw_orders AS (
  ...
  WHERE target_day BETWEEN DATE '2026-06-01' AND DATE '2026-06-30'
  ...
  SELECT
    successful_orders.user_id, user_profile.register_date, user_profile.first_pay_date,
...
SELECT
  ...
  COUNTIF(first_pay_date < DATE '2026-06-01') AS unique_old_payers_at_period_start,
