...
WHERE app_id = 90006
  ...
  AND (
    LOWER(field_name) LIKE '%channel%'
    OR LOWER(field_name) LIKE '%media%'
    OR LOWER(field_name) LIKE '%package%'
    OR LOWER(field_name) LIKE '%campaign%'
  )
GROUP BY field_name, field_value, action_type
HAVING users >= 30
ORDER BY users DESC;
